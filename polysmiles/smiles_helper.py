"""
a modified library of pysmiles
...
POLYMER_PATTERN=r'(?P<element>Pol.*|Q.*)?'
POLYMER_PATTERN=re.compile(r'^\['+POLYMER_PATTERN+r'\]$')


ATOM_PATTERN = re.compile(r'^\['+
                          ISOTOPE_PATTERN + ELEMENT_PATTERN +
...
def parse_atom(atom):
    ...
    if match is None:
        #parse with polymer pattern
        match = POLYMER_PATTERN.match(atom)
        if match is None:
